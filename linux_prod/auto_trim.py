...
Replaces the broken auto-trim.sh (which had a fatal heredoc-in-substitution bug at line 159).

Reads Hive boxes, calls Qwen for relevance scoring, compresses or evicts cold entries.
...
# ─── Configuration ─────────────────────────────────────────────
BASE_DIR = Path(__file__).resolve().parent.parent  # ai_desktop_assistant/
WORKSPACE = Path(os.environ.get("WORKSPACE", str(BASE_DIR.parent)))
...
def compress_block(text: str, model: str = TARGET_MODEL) -> dict:
    """Compress a single text block via Qwen. Returns dict with result or error."""
...
def write_signal(filename: str, data: dict) -> None:
    """Write a signal file for the Flutter bridge to pick up."""
...
def main():
    ...
    # Read context blocks (simplified — in production this reads from Hive/ContextEngine)
