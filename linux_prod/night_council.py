...
def run_session_ingestion(report):
    """Ingest unprocessed session dumps into Memory Palace via Ollama cloud."""
    try:
        sys.path.insert(0, os.path.join(os.path.expanduser("~/.hermes/scripts")))
        from session_ingest import run_ingestion

        stats = run_ingestion()
        report["session_ingestion"] = {
            "processed": stats["processed"],
            "success": stats["success"],
            "failed": stats["failed"],
        }
    except Exception as e:
        report["session_ingestion_error"] = str(e)
        logger = logging.getLogger("night_council")
        logger.error("Session ingestion failed: %s", e)
...
def run():
    ...
    # Ingest unprocessed session dumps into Memory Palace
    run_session_ingestion(report)
